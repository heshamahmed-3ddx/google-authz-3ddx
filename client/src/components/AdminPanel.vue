<template>
  <v-card class="mb-6" elevation="3">
    ...
    <v-dialog v-model="showAddPolicy" max-width="500px">
      <v-card>
        <v-card-title>
          <span class="text-h5">{{ $t('admin.addNewPolicy') || 'Add New Policy' }}</span>
        </v-card-title>
        <v-card-text>
    ...
    <v-dialog v-model="showAddAssignment" max-width="500px">
      <v-card>
        <v-card-title>
          <span class="text-h5">{{ $t('admin.addGroupAssignment') || 'Add Group Assignment' }}</span>
        </v-card-title>
        <v-card-text>
    ...
    <v-dialog v-model="showEditUser" max-width="600px">
      <v-card v-if="editingUser">
        <v-card-title>
          <span class="text-h5">{{ $t('admin.editUser') || 'Edit User' }}: {{ editingUser.email }}</span>
        </v-card-title>
        <v-card-text>
          <h4 class="mb-3">{{ $t('admin.currentGroups') || 'Current Groups' }}</h4>
          <v-chip-group>
            <v-chip 
          ...
          <h4 class="mt-4 mb-3">{{ $t('admin.availableGroups') || 'Available Groups' }}</h4>
          <v-chip-group>
            <v-chip 
...
const fetchUsers = async () => {
  ...
  try {
    ...
    if (error.response?.status === 404) {
      console.warn('Admin API endpoints not available. Server may need restart.')
    }
  } finally {
    loading.users = false
  }
...
const removeUserGroup = async (userEmail, group) => {
  try {
    ...
    // Update editing user
    if (editingUser.value && editingUser.value.email === userEmail) {
      editingUser.value.groups = editingUser.value.groups.filter(g => g !== group)
    }
  } catch (error) {
