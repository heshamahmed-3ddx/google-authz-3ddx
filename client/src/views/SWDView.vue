    ...
    <v-row>
      <v-col cols="12">
        ...
        <v-alert type="success" class="mb-4"
          >Welcome, SWD group member!</v-alert
        >
        <v-card v-if="loading" class="pa-4 text-center">
          <v-progress-circular indeterminate color="primary" size="32" />
          <div class="mt-2">Loading SWD group info...</div>
        </v-card>
...
onMounted(async () => {
  try {
    // Use the same endpoint as DashboardView for user details (includes Google group info)
