  ...
  page.on('response', async (res) => {
    try {
      ...
      if (status >= 400) {
        ...
        failedResponses.push({ url: res.url(), status, statusText: res.statusText(), bodySnippet: txt.slice(0, 1000) })
      }
    } catch (e) {
      // ignore
    }
...
try {
  ...
  page.on('console', msg => {
    ...
    if (text.startsWith('[Vue warn]') || text.includes('onScopeDispose() is called') || text.includes('Missing ref owner context') || text.includes('withDirectives can only be used')) {
      return
    }
  ...
  await page.goto(TARGET_URL, { waitUntil: 'networkidle2', timeout: 30000 })
    ...
    try {
      await page.waitForFunction(() => {
        ...
        if (window.__APP__ && window.__APP__.router && window.__APP__.router.currentRoute) {
          try {
            return window.__APP__.router.currentRoute.value && typeof window.__APP__.router.currentRoute.value.fullPath === 'string'
          } catch (e) {
            return false
          }
        }
      ...
      console.warn('Router did not report ready state within timeout; proceeding anyway')
    }
    ...
    results.forEach(r => console.log(`- ${r.key}: ${r.found ? 'OK' : 'MISSING'} ("${r.expected}")`))
    ...
    if (!allOk) {
      ...
      try {
        const routerInfo = await page.evaluate(() => {
          ...
          if (window.__APP__ && window.__APP__.router) {
            ...
            const routes = (r.getRoutes && typeof r.getRoutes === 'function') ? r.getRoutes().map(rt => ({ name: rt.name, path: rt.path })) : []
            const matched = (r.currentRoute && r.currentRoute.value && r.currentRoute.value.matched) ? r.currentRoute.value.matched.map(m => ({ name: m.name, path: m.path })) : []
            // Inspect resolved components if available
            const matchedComponents = (r.currentRoute && r.currentRoute.value && r.currentRoute.value.matched) ? r.currentRoute.value.matched.map(m => {
              const comps = m.components || {}
              const info = {}
              Object.keys(comps).forEach(k => {
                const c = comps[k]
                info[k] = { type: typeof c, str: (typeof c === 'function') ? (c.toString().slice(0,200)) : (typeof c === 'object' ? 'object' : String(c)) }
              })
              return { name: m.name, path: m.path, components: info }
            }) : []

            return { currentRoute: r.currentRoute.value.fullPath, isReady: !!r.isReady && typeof r.isReady === 'function' ? r.isReady() : null, routes, matched, matchedComponents }
          }
          return null
        })
      ...
      if (failedResponses.length > 0) {
        ...
        failedResponses.forEach(r => {
          console.error(`${r.status} ${r.url} -> ${r.bodySnippet ? r.bodySnippet.replace(/\n/g, ' ') : ''}`)
        })
      ...
      try {
        ...
        const checkKey = (dot) => dot.split('.').reduce((o,k)=> (o && o[k]!==undefined) ? o[k] : undefined, msgs)
        const fileResults = results.map(r => {
          const val = checkKey(r.key)
          console.error(`${r.key}: ${val ? 'PRESENT' : 'MISSING'} ${val ? `("${String(val)}")` : ''}`)
      ...
      if (translationFallbackOk) {
        console.log('Runtime rendering failed but translation file contains all keys — treating as PASS')
        process.exit(0)
      }
    }
